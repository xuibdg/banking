...
//            }
    ...
    //                .releaseAccountNumber(savingAccount.getAccountNumber())
        ...
        private String createDisbursementJournal(LoanAccount loanAccount, BigDecimal amount, UserMetaData userMetaData) {
            List<JournalDetailRequest> details = buildLoanDisbursementDetails(loanAccount, amount);
    
            JournalRequest journalRequest = buildJournalRequest("LOAN_DISBURSEMENT", "Pencairan Pinjaman - " + loanAccount.getAccountNumber(), details
            );
        ...
        private String getCoaId(String coaCode) {
            return mChartOfAccountRepository.findByCode(coaCode).orElseThrow(() -> new BusinessException(HttpStatus.BAD_REQUEST, GlobalErrorMapping.COA_NOT_FOUND)).getId();
        }
        private JournalDetailRequest buildJournalDetail(String coaCode, String mutationType, BigDecimal amount, String description) {
            JournalDetailRequest detail = new JournalDetailRequest();
            detail.setCoaId(getCoaId(coaCode));
            detail.setMutationType(mutationType);
            detail.setAmount(amount);
            detail.setDescription(description);
            return detail;
        }
        private JournalRequest buildJournalRequest(String referenceType, String description, List<JournalDetailRequest> details) {
        ...
        private List<JournalDetailRequest> buildLoanDisbursementDetails(LoanAccount loanAccount, BigDecimal amount) {
            String accountRef = loanAccount.getAccountNumber();
    
            return Arrays.asList(
                    buildJournalDetail(COA_PIUTANG_PEMBIAYAAN, "DEBIT", amount, "Pencairan pinjaman - " + accountRef),
                    buildJournalDetail(COA_TABUNGAN_NASABAH, "CREDIT", amount, "Pencairan pinjaman ke tabungan - " + accountRef)
            );
        }
    
        private List<JournalDetailRequest> buildLoanRepaymentDetails(LoanAccount loanAccount, BigDecimal totalPayment, BigDecimal principalComponent, BigDecimal interestComponent, BigDecimal feeComponent) {
            String accountRef = loanAccount.getAccountNumber();
            List<JournalDetailRequest> details = new ArrayList<>();
    
            details.add(buildJournalDetail(COA_TABUNGAN_NASABAH, "DEBIT", totalPayment, "Pembayaran cicilan - " + accountRef));
    
            details.add(buildJournalDetail(COA_PIUTANG_PEMBIAYAAN, "CREDIT", principalComponent, "Pembayaran pokok - " + accountRef));
    
            details.add(buildJournalDetail(COA_PENDAPATAN_BUNGA_LOAN, "CREDIT", interestComponent, "Pendapatan bunga - " + accountRef));
    
            if (feeComponent.compareTo(BigDecimal.ZERO) > 0) {
                details.add(buildJournalDetail(COA_PENDAPATAN_BIAYA_ADMIN, "CREDIT", feeComponent, "Pendapatan biaya admin - " + accountRef));
            }
